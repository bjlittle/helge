...
|---|---|---|---|
| The lifecycle names `.gravity/verification.md` as the source of verification commands, but it records none yet, and the policy reserves `.gravity/policy.md` for human edits without saying who edits the rest of `.gravity`. | Lifecycle, verification | Bill Little | Resolved: the owner approved on 2026-09-17 that the agent edits `.gravity/verification.md` directly. The build pull request added the commands to both `AGENTS.md` and `.gravity/verification.md` — but as a table, which `scripts/sdlc doctor` cannot see, since it reads commands from lines beginning with `- `. The gate went on reporting none recorded until 2026-09-18, when the file was returned to a list and the empty `verify_*` answers were filled in. Resolved for a reader on 2026-09-17; resolved for the gate on 2026-09-18. |
